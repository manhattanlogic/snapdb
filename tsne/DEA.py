...
'''
class DEA:
    def __init__(self, layer_shapes=[100, 32, 4, 2], batch_size=256,
        ...
        for a in range (1, len(layer_shapes)):
            ...
            ae["hidden"] =  tf.tanh(tf.matmul(ae["input"], self.weights[a-1][0]) + self.weights[a-1][1])
        ...
        for i in range(1, len(self.weights)+1):
            hidden = tf.matmul(self.ae["layers"][-1], self.weights[i-1][0]) + self.weights[i-1][1]
